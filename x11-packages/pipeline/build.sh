...
TERMUX_PKG_DEPENDS="glib, gtk4, clapper, clapper-enhancers, sqlite, openssl, ca-certificates, python-yt-dlp, deno, ffmpeg"
...
termux_step_pre_configure() {
    ...
    # Tell Cargo to cross-compile since pipeline's meson.build invokes cargo directly
    ...
    # Append to target-specific RUSTFLAGS to avoid overriding Termux's rpath settings
