...
_CHROMIUM_VERSION=144.0.7559.134
TERMUX_PKG_VERSION=40.4.0
...
termux_step_post_get_source() {
	...
	local f
	for f in $(find "$TERMUX_PKG_BUILDER_DIR/../electron-host-tools-for-vesktop/cr-patches" -maxdepth 1 -type f -name *.patch | sort); do
	...
	local f
	for f in $(find "$TERMUX_PKG_BUILDER_DIR/../electron-host-tools-for-vesktop/electron-patches" -maxdepth 1 -type f -name *.patch | sort); do
		echo "Applying patch: $(basename $f)"
		patch --silent -p1 <"$f"
	done
	...
	local f
	for f in $(find "$TERMUX_PKG_BUILDER_DIR/../electron-host-tools-for-vesktop/jumbo-patches" -maxdepth 1 -type f -name *.patch | sort); do
		echo "Applying patch: $(basename $f)"
		patch --silent -p1 <"$f"
	done
...
# Use prebuilt js2c
prebuilt_js2c_binary = \"$TERMUX_PREFIX/opt/electron-host-tools-for-vesktop/$_v8_toolchain_name/node_js2c\"
" >>$_common_args_file
...
termux_step_make_install() {
	cd $TERMUX_PKG_BUILDDIR
	local _install_prefix=$TERMUX_PREFIX/lib/vesktop
	...
	mv $_install_prefix/electron $_install_prefix/vesktop
